...
PERPLEXITYAI_MODEL = os.environ.get('PERPLEXITYAI_MODEL', 'gpt-3.5-turbo-16k')
...
# Set up PERPLEXITYAI API
PERPLEXITYAI.api_key = PERPLEXITYAI_API_KEY
...
async def chat_with_PERPLEXITYAI(message):
    ...
    try:
        response = PERPLEXITYAI.ChatCompletion.create(
    ...
    except PERPLEXITYAI.error.PERPLEXITYAIError as e:
        typing_task.cancel()
        await message.channel.send(f"An error occurred: {str(e)}")
        logger.error(f"PERPLEXITYAI API error for user: {message.author.name} (ID: {message.author.id}). Error: {str(e)}")
